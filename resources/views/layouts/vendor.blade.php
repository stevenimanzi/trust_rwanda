    ...
    <div class="hz-top-actions d-flex align-items-center gap-2 gap-md-3">
        <div class="d-flex align-items-center gap-2">
            ...
            <div class="dropdown">
                <div class="hz-icon-btn-light" data-bs-toggle="dropdown" aria-expanded="false" style="cursor: pointer;">
                    ...
                    @if($vendorPendingOrders > 0)
                        <span class="hz-dot" style="background:#ef4444; width:8px; height:8px; top:8px; right:8px; position:absolute; border-radius:50%; border: 2px solid white;"></span>
                    @endif
                </div>
                <ul class="dropdown-menu dropdown-menu-end shadow border-0 mt-2 p-0" style="width: 320px; border-radius: 12px; overflow: hidden;">
                    <li class="px-3 py-3 border-bottom bg-light fw-bold text-dark d-flex justify-content-between align-items-center">
                        ...
                        @if($vendorPendingOrders > 0)<span class="badge bg-danger rounded-pill">{{ $vendorPendingOrders }} New</span>@endif
                    </li>
                    @if($vendorPendingOrders > 0)
                    <li>
                        <a class="dropdown-item py-3 d-flex align-items-start gap-3" href="{{ route('vendor.orders') }}" style="white-space: normal;">
                            <div class="bg-warning-subtle text-warning p-2 rounded-circle mt-1"><i class="bi bi-box-seam"></i></div>
                            <div>
                                <div class="fw-bold text-dark mb-1">New Pending Orders</div>
                                <div class="small text-muted" style="line-height: 1.4;">You have {{ $vendorPendingOrders }} order(s) awaiting your action. Please review them.</div>
                            </div>
                        </a>
                    </li>
                    @else
        ...
        <div class="dropdown">
            <div class="d-flex align-items-center gap-2 ms-1 ms-md-2 cursor-pointer" data-bs-toggle="dropdown" aria-expanded="false">
                <img src="https://ui-avatars.com/api/?name={{ urlencode($vendorUser->name ?? 'Robert Fox') }}&background=1e293b&color=ffffff&bold=true" class="hz-avatar rounded-circle" style="width: 40px; height: 40px;" alt="User Avatar">
                <div class="d-none d-md-block">
                    <div class="fw-bold text-dark" style="font-size:0.9rem; line-height: 1;">{{ explode(' ', $vendorUser->name ?? 'Robert Fox')[0] }}</div>
                    <div class="text-muted" style="font-size:0.75rem;">Vendor</div>
                </div>
            ...
            <ul class="dropdown-menu dropdown-menu-end shadow border-0 mt-2" style="border-radius: 12px; min-width: 200px;">
                <li class="px-3 py-2 border-bottom mb-1">
                    <div class="fw-bold text-dark">{{ $vendorUser->name ?? 'Robert Fox' }}</div>
                    <div class="small text-muted">{{ $vendorUser->email ?? 'vendor@example.com' }}</div>
                </li>
                <li><a class="dropdown-item py-2 d-flex align-items-center" href="{{ route('vendor.settings') }}"><i class="bi bi-gear me-3 text-muted"></i> Store Settings</a></li>
